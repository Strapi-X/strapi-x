...
class Converter {
    ...
    constructor(strapiModelsParse, config) {
        ...
        this.strapiModels = strapiModelsParse.map((m) => {
            ...
            const interfaceName = pascalCase(m.info.singularName);
            const ouputFile = `${paramCase(m.info.singularName)}.model`;
    ...
    async run() {
        return new Promise((resolve, reject) => {
            ...
            this.strapiModels.forEach(g => {
                ...
                const modelFolder = path.resolve(this.config.output, '../', g.ouputFile);
                if (!fs.existsSync(`${modelFolder}.ts`)) {
                    fs.writeFile(`${modelFolder}.ts`, this.modelToInterface(g), { encoding: 'utf8' }, (err) => {
    ...
    strapiModelToInterface(m) {
        ...
        result.push('   */');
        result.push(`   export class ${pascalCase(m.interfaceName)} {`);
        ...
        result.push(`  `);
        result.push(`       constructor(${camelCase(m.info.singularName)}: Partial<${pascalCase(m.interfaceName)}>) {`);
        result.push(`           Object.assign(this, ${camelCase(m.info.singularName)});`);
        result.push(`       }`);
        result.push('   }');
        result.push('}');
    ...
    modelToInterface(m) {
        ...
        result.push(`import { Strapi } from './strapi/${paramCase(m.interfaceName)}.model';`);
        result.push('');
        result.push('/**');
        result.push(`* Model definition for ${m.info.displayName}`);
        result.push('*/');
        result.push(`export class ${pascalCase(m.interfaceName)} extends Strapi.${pascalCase(m.interfaceName)} {`);
        result.push(``);
        result.push(`   constructor(${camelCase(m.info.singularName)}: Partial<${pascalCase(m.interfaceName)}>) {`);
        result.push(`       super(${camelCase(m.info.singularName)});`);
        result.push(`       Object.assign(this, ${camelCase(m.info.singularName)});`);
        result.push(`   }`);
        result.push('}');
 ...
 */
 export const convert = async (strapiModels, config) => {
