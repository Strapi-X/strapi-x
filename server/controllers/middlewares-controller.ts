...
import { MiddlewareConfiguration } from '../models/middleware-configuration';
...
export default ({ strapi }: { strapi: Strapi; }) => ({
  ...
  saveMiddlewaresConfigurations(ctx) {
    const body: { route: string, middleware: MiddlewareConfiguration; }[] = ctx.request.body;
  ...
  deleteMiddlewaresConfigurations(ctx) {
    const body: MiddlewareConfiguration[] = ctx.request.body;
    const extension: MiddlewaresConfiguration = require(path.resolve(strapi.dirs.app.src, 'extensions/strapi-x/middlewares/configuration.js'));
